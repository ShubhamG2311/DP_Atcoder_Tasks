...
//------------------------------------Let's Go!!--------------------------------------------------------------------------------------------------------
ll knapsack(vi &weight, vi &val, ll n, ll w,vector<vector<ll>> &dp)
{
  if (n<0) return 0;
  if (n==0) 
  {
    if (weight[n]<=w) return val[n];
    else return 0;
  }
  if (weight[n]<=w) 
  {
    if (dp[n-1][w-weight[n]]==-1) dp[n-1][w-weight[n]]=knapsack(weight,val,n-1,w-weight[n],dp);
    if (dp[n-1][w]==-1) dp[n-1][w]=knapsack(weight,val,n-1,w,dp);
    return max((val[n]+dp[n-1][w-weight[n]]),dp[n-1][w]);
  }
  else 
  {
    if (dp[n-1][w]==-1) dp[n-1][w]=knapsack(weight,val,n-1,w,dp);
...
void solve()
{
  ...
  cout<<knapsack(weight,val,n-1,w,dp);
}
